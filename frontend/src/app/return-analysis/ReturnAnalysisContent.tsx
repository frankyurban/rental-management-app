...
export default function ReturnAnalysisContent() {
  ...
  useEffect(() => {
    if (!authLoading && !user) {
      ...
      // Redirect non-owners to the homepage
      router.push('/')
    }
  }, [user, authLoading, router])

  // Pull the id from the URL, if present
  const idFromUrl = searchParams.get('id')
  const [selectedId, setSelectedId] = useState<number | ''>(idFromUrl ? Number(idFromUrl) : '')
  ...
  useEffect(() => {
    if (selectedId) {
      ...
      fetch(`/api/properties/${selectedId}/analysis`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      })
  ...
  // If user navigates to the page with a new id in the URL, update selectedId
  useEffect(() => {
    if (idFromUrl && Number(idFromUrl) !== selectedId) {
  ...
  async function handleSave() {
    if (!selectedId) {
      setMessage('Select a property to save analysis.')
      return
    }
    setSaving(true)
    setMessage('Saving...')
    try {
      ...
      const res = await fetch(`/api/properties/${selectedId}/analysis`, {
        method: 'POST',
        headers: { 
  ...
  return (
    <div className="max-w-4xl mx-auto py-10 px-2 print:bg-white print:text-black">
      <h1 className="text-3xl font-bold mb-6">Return Analysis</h1>
      <div className="flex flex-wrap gap-4 items-end mb-6">
        <div>
          <label className="block text-xs font-semibold mb-1" htmlFor="property">Property</label>
          <select
            id="property"
            value={selectedId ?? ''}
            onChange={e => setSelectedId(e.target.value ? Number(e.target.value) : '')}
            className="border px-3 py-2 rounded min-w-64 bg-white"
